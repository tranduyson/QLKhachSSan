...
namespace QLKhachSanApi.Repositories
{
    public class ChiTietDatPhongAdoRepository : IRepository<ChiTietDatPhong>
    {
        ...
        public async Task<IEnumerable<ChiTietDatPhong>> GetAllAsync()
        {
            ...
            using (var cmd = new SqlCommand("SELECT MaCT, MaDatPhong, MaPhong, DonGia, SoDem, ThanhTien FROM ChiTietDatPhong", conn))
            {
        ...
        {
            using (var conn = _db.GetConnection())
            using (var cmd = new SqlCommand("SELECT MaCT, MaDatPhong, MaPhong, DonGia, SoDem, ThanhTien FROM ChiTietDatPhong WHERE MaCT=@Id", conn))
            {
...
OUTPUT INSERTED.MaCT, INSERTED.MaDatPhong, INSERTED.MaPhong, INSERTED.DonGia, INSERTED.SoDem, INSERTED.ThanhTien
VALUES(@MaDatPhong, @MaPhong, @DonGia, @SoDem)", conn))
            {
...
VALUES(@MaDatPhong, @MaPhong, @DonGia, @SoDem)", conn, tx))
        ...
        {
            using (var conn = _db.GetConnection())
            using (var cmd = new SqlCommand(@"UPDATE ChiTietDatPhong SET MaDatPhong=@MaDatPhong, MaPhong=@MaPhong, DonGia=@DonGia, SoDem=@SoDem WHERE MaCT=@Id", conn))
            {
        ...
        public async Task<IEnumerable<ChiTietDatPhong>> GetByDatPhongAsync(int maDatPhong)
        {
            var list = new List<ChiTietDatPhong>();
            using (var conn = _db.GetConnection())
            using (var cmd = new SqlCommand("SELECT MaCT, MaDatPhong, MaPhong, DonGia, SoDem, ThanhTien FROM ChiTietDatPhong WHERE MaDatPhong=@MaDatPhong", conn))
            {
